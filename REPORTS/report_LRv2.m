allenamen = ('C:\Users\maipa\Desktop\Alltogether\MoTrack_Ready_Test\Running_Shoe_A')
files = dir(allenamen)
files = files(arrayfun(@(x) ~strcmp(x.name(1),'.'),files));

for ff=[37]%1:length (files) %[]%length (files) 


clc
clearvars -except files allenamen ff
close all
probID = files(ff).name
probmatthias = strrep(probID,'P','p')

matthias_left = strcat (probmatthias,'_l');
druckbilder= 0; % 0 ohne 1 mit wenn 0 geht es schnellerF5
...
try
    cd ( strcat('C:\Users\maipa\Desktop\Reports\', probID))
catch
    mkdir  ( strcat('C:\Users\maipa\Desktop\Reports\', probID))
    cd ( strcat('C:\Users\maipa\Desktop\Reports\', probID))
end
...
paraZiel = Paragraph(['Ziel der Studie, an der Sie als Proband*in teilgenommen haben, ist es, den Einfluss von verschiedenen Mittelsohlensteifigkeiten auf die Laufmechanik zu bestimmen.  Hierfür wurde mit Hilfe eines 3D-Bewegungsanalysesystems die Bewegung des Körpers beim Laufen aufgenommen. Die aufgezeichneten Daten sollen uns unter Anderem dabei helfen ein besseres Verständnis bezüglich wichtiger biomechanischer Parameter, die für die Herstellung von individuellen Laufschuhen benötigt werden, zu erlangen. Zu Beginn der Untersuchung wurden Ihnen 78 reflektierende Marker an definierten anatomischen Punkten auf die Haut aufgeklebt. Diese Marker wurden von unserem Infrarotlicht-basierten Kamerasystem detektiert und aufgenommen. Mithilfe der digitalisierten Markerpositionen konnten wir später die Bewegungsanalyse durchführen.'...
    ' Des Weiteren wurden Bodenreaktionskräfte entlang der Laufstrecke mittels Kraftmessplatten, welche in den Boden eingelassen waren, erfasst. Zusätzlich wurde die Druckverteilung des Fußes beim Gehen und langsamen Laufen erfasst. Außerdem wurde die Anthropometrie des Fußes und des gesamten Körpers mittels eines 3-dimensionalen Oberflächenscans erfasst. Darüber hinaus wurden maximale Muskelkräfte der Zehenflexoren während isometrischer Bedingungen bestimmt. Diese zusätzlichen Messungen sollten uns dabei helfen ein besseres Verständnis darüber zu erlangen, welche Einflussfaktoren das individuelle Laufverhalten beeinflussen können.']);
...
paraInfoZehen = Paragraph(['Eine Vielzahl wissenschaftlicher Publikationen hat gezeigt, dass die Zehenkraft in Korrelation mit der Gesamtstabilität des Fußes steht. Zudem hat sie Einfluss auf die Deformation des Fußgewölbes während des Gehens und Laufens. Eine stärkere Zehenkraft führt in der Regel zu einer geringeren Deformation des Fußgewölbes und kann so Laufverletzung präventiv beeinflussen. Des Weiteren ist die Zehenkraft auch ein wichtiger Performanceindikator. Beispielsweise hat die Zehenkraft Einfluss auf das Abdruckverhalten beim Laufen und Springen. Diese Kraft wird unter anderem aus der extrinsischen Fußmuskulatur (Wadenmuskutlatur) generiert und über Sehnen auf das Zehengrundgelenk übertragen. ']);


if ff==29;
    paraEMEDinfo = Paragraph(['Mittels der aus insgesamt über 6000 Sensoren bestehenden Druckmessplatte kann die Druckverteilung des Fußes sowohl während dynamischer als auch statischer Bewegungen gemessen werden. Im Gegensatz zu den statischen Messungen haben dynamische Messungen den Vorteil, dass das Gangbild in unterschiedlichen Bewegungsphasen genau beobachtet werden kann. So kann beispielweise die Verformung des Fußgewölbes während der Bewegung bestimmt werden. Unter anderem kann somit auch ein Links-Rechts-Vergleich der Druckbilder während der Belastung aufgezeigt werden. Dies kann asymmetrische Druckbilder identifizieren. Bei ihrem Abdruckmuster kommt es zu einem deutlich verstärkten Abdruck über den rechten Fuß.']);
...
paraGRF = Paragraph(['In der Biomechanik können wir die Kräfte, die von außen auf einen Menschen wirken, beim Laufen sehr genau messen. Diese Kräfte, auch externe Kräfte genannt, können beispielweise mittels in den Boden eingelassenen Kraftmessplatten erfasst werden. Die Kraft kann somit als eine 3D-Größe beschrieben werden. Man unterscheidet in vertikale Bodenreaktionskraft, Reaktionskräfte die seitlich wirken (medial zur Körpermitte hin / lateral von der Körpermitte weg), und Kräfte die nach vorn und hinten wirken (anterior / posterior). Die Kräfte werden für die Berechnung von Gelenksbelastung (Drehmomente) benötigt. Weitere Informationen hierzu finden Sie im nachfolgenden Kapitel.']);
paraKINEMATIKinfo = Paragraph(['Die Gelenkskinematik beschreibt die Bewegung verschiedener benachbarter Körpersegmente (z.B. Ober- und Unterschenkel) im 3D-Raum. Somit können Gelenkswinkel in allen 3 Raumachsen beschrieben werden. Man unterscheidet zwischen Bewegungen in der Sagittal- (Flexion, Extension), der Frontal- (Abduktion, Adduktion) und der Transversalebene (interne, externe Rotation). Zusätzlich sei zu erwähnen, dass die Bewegung in der Fontaleben des Fußes oftmals mit den Begriffen Eversion und Inversion beschrieben werden.']);
paraKINETIKinfo = Paragraph(['Das Gelenksdrehmoment ist das Produkt aus Kraft mal Hebelarm und stellt die Belastung am Gelenks während der Bewegung dar. Drehmomente, wie beispielsweise das Knieabduktionsmoment zeigen einen Zusammenhang mit Überbelastungsverletzungen im Laufsport. In einer Vielzahl von wissenschaftlichen Studien wurde jedoch auch schon gezeigt, dass sich Drehmomente an Gelenken durch Veränderungen des Schuhwerks erzielen lassen. Die untenstehende Grafik soll dieses Prinzip verdeutlichen. Die (externe) Kraft und die Entfernung zum Gelenkszentrum (Hebelarm) erzeugen ein Drehmoment (extern), dieses Drehmoment lässt sich durch eine Umorientierung der Kraft (Veränderung des Kraftangriffspunkt, Schnittstelle Laufschuh und Boden) verändern. Bitte beachten Sie, dass wir im Folgenden auf die internen Drehmomente der Gelenke verweisen. Interne Momente werden durch Bänder, Sehnen und Muskeln erzeugt und wirken den externen Drehmomenten entgegen. Sie sind daher nur in ihrem Vorzeichen vertauscht.']);
paraSCHUHINFO = Paragraph(['In dieser Studie wurden unter anderem der Einfluss unterschiedlicher Schuhe auf die Laufbiomechanik getestet. Dabei handelte es sich um eine Blindstudie. Das heißt, Sie als Teilnehmer*in wussten zu dem Zeitpunkt der Messungen nicht, welche Faktoren an den Laufschuhen verändert worden sind. Im Nachhinein soll nun auf drei der ingesamt sieben getesteten Schuhe näher eingegangen werden. Die Schuhe A, B und C sind zur Veranschaulichung in der untenstehenden Grafik dargestellt. In dieser sehen Sie die drei Schuhbedingungen (Mittelsohlen) in der Draufsicht. Dabei wurde systematisch die Härteverteilung im Rückfuss verändert. Die schwarzen Punkte zeigen hierbei harte Elemente, grau weniger harte und weiß weiche Elemente. Die genauen Härtegrade sind in Rockwell (C) angeben. Mittels dieser Methode lassen sich so beispielsweise individualisierte Laufschuhe  herstellen und so möglicherweise laufbedingte Überbelastungsverletzungen reduzieren.']);
para3DBewegungInfo = Paragraph(['Bei dieser Studie wurden verschiedene Systeme zur Erfassung von Bewegungen des menschlichen Bewegungsapparates („Motion Capturing“) eingesetzt. Diese Systeme messen die drei Säulen der Biomechanik. Diese drei Säulen bilden die Körpermaße (Anthropometrie), die Bewegung (Kinematik) und die Kraft (Kinetik). Die erfassten Daten wurden zeitsynchron aufgezeichnet, um sie später in ihrem Zusammenhang auswerten zu können. Die Körpermaße sind die Grundlage für die Auswertung des verwendeten Marker-basierten Kamerasystems, welches die dreidimensionale Bewegung des Körpers und der Extremitäten im Raum auf einige Millimeter genau ermitteln kann. Werden zusätzlich zeitgleich die Bodenreaktionskräfte mittels 3D Kraftmessplatten gemessen, kann über Kalkulationen der Gelenkmittelpunkt, Kraftangriffspunkte und demnach Hebelarme und Gelenkbelastung gemessen werden. Die Gelenkbelastung ist somit abhängig von auftretenden Kräften und Bewegungsveränderungen, die maßgeblich durch das Schuhwerk beeinflusst werden können. Mit Hilfe von Motion Capture Systemen können somit Aussagen zum Einfluss der verschiedenen Laufschuhe auf den Bewegungsapparat beispielsweise zur Verletzungsprävention oder der Optimierung der Performance getroffen werden.']);


%%%%%% end of additional text
rpt = Report(probID,'pdf');
tp = TitlePage;
tp.Title = 'Studie zum individuellen Laufverhalten und zur Individualisierung der Laufschuhtechnologie basierend auf systematischer Veränderung der Mittelsohlensteifigkeit ';
tp.Subtitle = '3-Dimensionale Bewegungsanalyse';
tp.Author = '';
% Create a sample image
% im = Image(which('IMAGES_REPORT\Logo_DSHS_Koeln.png'));
% im.Width = '0.75in';
% im.Height = '0.75in';
...
figure()
tp.Image =('C:\Users\maipa\Desktop\IMAGES_REPORT\running_man.png');
axis('image');
% set(gca,'Xtick',[],'Ytick',[],...
...
imshow ('C:\Users\maipa\Desktop\IMAGES_REPORT\bmi.png')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ch3 = Chapter();
ch3.Title = sprintf('Zehenkraft, Druckverteilung und Anthropometrie');
...
if gender =='m'
    h3= scatter (2,zehenkraftprob,100, 'ro', 'filled');
         text(2,zehenkraftprob-zehenkraftprob*0.05,num2str(round(zehenkraftprob,2)))
...
%
figZehen.Snapshot.Caption = ('Die obenstehende Grafik zeigt die Mittelwerte der Zehenkraft getrennt nach Frauen und Männern an. Der rote Punkt zeigt Ihre Zehenkraft im Vergleich zum jeweiligen Gruppenmittelwert (alle Läuferinnen / Läufer in unsere Studie).');
...
if ff==29
    paraZehenWertung = Paragraph([(strcat('Ihre Zehenkraft ist im Vergleich zu den anderen Männern  ist ähnlich wie der Gruppenmittelwert. Trotz dessen könnten Sie von einer weiteren Stärkung der Fußmuskulatur profitieren. Um Ihre Zehenkraft zu trainieren empfehlen wir folgende Übungen https://www.runnersworld.de/krafttraining-stretching/fussmuskulatur-staerken/'))]);

else
paraZehenWertung = Paragraph([(strcat('Ihre Zehenkraft ist im Vergleich zu den anderen',{' '},moderw,{' '}, '', {' '},convertCharsToStrings(temptext), {' '},'als der Gruppenmittelwert.',{' '}, zusatztext))]);
end
...
ImagefuesseStatic =('C:\Users\maipa\Desktop\IMAGES_REPORT\emed_final.png');
...
figDruckFancy.Snapshot.Caption = ('Maximaler Druck in den verschiedenen Fußregionen im Links-Rechts Vergleich. Bitte beachten Sie, dass die Grafik nur für Visualisierungszwecke verwendet wurde. Die Druckwerte (kPa) entsprechen jedoch Ihren persönlichen Werten.');
...
vector(1,1)= nanmean ((proz1(:,2)))*100 %verserechtslat
vector(2,1)= nanmean ((proz1(:,1)))*100 %verserechtsmed
vector(3,1)= nanmean(sum(proz1(:,3:4),2))*100 %mittelfussrechts
vector(4,1)= nanmean ((proz1(:,5)))*100 %metarechts1
vector(5,1)= nanmean ((proz1(:,8)))*100 %bigtoerechts
subplot(1,2,2)
pie (vector)
...
vectorL(1,1)= nanmean ((proz1l(:,2)))*100 %verserechtslat
vectorL(2,1)= nanmean ((proz1l(:,1)))*100 %verserechtsmed
vectorL(3,1)= nanmean(sum(proz1l(:,3:4),2))*100 %mittelfussrechts
vectorL(4,1)= nanmean ((proz1l(:,5)))*100 %metarechts1
vectorL(5,1)= nanmean ((proz1l(:,8)))*100 %bigtoerechts
subplot(1,2,1)
pie (vectorL)
...
Imagefuesseleg =('C:\Users\maipa\Desktop\IMAGES_REPORT\legendfuss_new.png');
...
fv.vertices=vnew;
fv.faces=fnew;
[Mtestung,Itestung] = (min(fv.vertices(:,end)))
 gross = abs(max(fv.vertices(:,end)))
 berechnteKR= convertCharsToStrings(num2str(abs((gross-abs(Mtestung))/1000)));
patch(fv,'FaceColor',       [0.8 0.8 1.0],  'EdgeColor',       'none',         'FaceLighting',    'gouraud',       'AmbientStrength', 0.15);
camlight('headlight');
material('dull');
axis('image');
...
para3DKoerperscanninfo= Paragraph([strcat('Sowohl der 3D- Körperscan als auch der 3D-Fussscan ermöglicht es genaue Angaben über die Körperoberfläche, die Massenmittelpunkte und weitere anthropometrische Eigenschaften des Körpers machen zu können. Diese können wiederum dazu genutzt werden, um zukünftig beispielsweise mittels Machine Learning Hervorsagen über Verletzungsrisikos und Laufpräferenzen machen zu können. Die Wissenschaft steht hierbei allerdings noch am Anfang der Forschung, sodass wir Ihnen hierzu keine weiteren Informationen geben können. Zudem haben wir diese Daten bisher noch nicht genauer analysieren können. ','Ihre berechnete Körpergröße im 3D Scan betrug', {' '}, berechnteKR,'m.')]);
...
imshow ('C:\Users\maipa\Desktop\IMAGES_REPORT\Rastergrafik_Schuhe.png')
...
figGRFZ.Snapshot.Caption = sprintf('Senkrecht / Vertikale Bodenreaktionskraft im Verlauf der Stützphase, angegeben als ein Vielfaches Ihres Körpergewichts. Die dünnen Linien zeigen dabei Gruppenmittelwert aller Probanden.');
...
figGRFX.Snapshot.Caption = sprintf('Posterior / anterior Bodenreaktionskraft im Verlauf der Stützphase, angegeben als ein Vielfaches Ihres Körpergewichts. Die dünnen Linien zeigen dabei Gruppenmittelwert aller Probanden.');
...
figGRFY.Snapshot.Caption = sprintf('Medial / Laterale Bodenreaktionskraft im Verlauf der Stützphase, angegeben als ein Vielfaches Ihres Körpergewichts. Die dünnen Linien zeigen dabei Gruppenmittelwert aller Probanden.');
...
imshow ('C:\Users\maipa\Desktop\IMAGES_REPORT\fussaufatz_small.png')
...
paraFussaufatz = Paragraph([strcat('Basierend auf dem Fussaufsatzverhalten kann man Läufer*innen klassifizieren. Ihr Fussaufatz betrug im Mittel', {' '},convertCharsToStrings(mittelfussaufatz),'°', {' '},'Sie sind also eher ein', {' '}, convertCharsToStrings(laufertype),'.',{' '},'Im Laufschuh A betrug Ihr Fussaufatzwinkel',schuhA,{'° '},', im Schuh B',{' '},SchuhB, {'° '}, 'und im Schuh C',{' '},SchuhC, {' °.'} , 'Ihr Fussaufsatzverhalten hat sich somit durch die Laufschuhe etwas verändert.')]);
...
paraEverVelcoity = Paragraph([strcat('Die maximale Eversionsgeschwindigkeit des Sprunggelenks gibt an, wie schnell die Eversion des Sprunggelenks stattgefunden hat. Dieser Parameter wird ebenfalls im Zusammenhang mit laufbedingten Überbelastungsverletzungen genannt. Die Eversionsgeschwindigkeit (negativer Wert in °/s) und die Inversionsgeschwindigkeit (postiver Wert in °/s) können auch durch verschiedene Schuhmodifikationen beeinflusst werden.  Ein Schuh, der dabei die Eversionsgeschwindigkeit verringert (weniger hohe negative Werte) kann somit unter Umständen das Verletzungsrisiko minimieren.',{' '},besti,{' '} , 'verringert bei Ihnen die Eversionsgeschwindigkeit.')]);
...
figeversionvel.Snapshot.Caption = sprintf('Ihre Maximale Sprunggelenks Eversionsgeschwindigkeit (rot), im Vergleich zu den anderen Teilnehmern.');
figeversionvel.Snapshot.Height = '3in';
figeversionvel.Snapshot.Width = '4in';
add(sec42,figeversionvel);  % add the figure to section
paraEverWinkel = Paragraph([strcat('Hohe maximale Eversionswinkel am Sprunggelenks werden ebenfalls oft in Verbindung mit Verletzungen im Laufsport erwähnt. Oftmals wird dies auch als Überpronation bezeichnet. Es sei an dieser Stelle jedoch zu erwähnen, dass die Studienlage hierzu sehr gegensätzliche Ergebnisse präsentiert. Höhere Eversionswinkel beudeuten dem zufolge nicht automatisch ein großes Verletzungsrisiko.')]);
...
plot(DATA.KandK.ProbMeanCurves.Running_Shoe_A.ANGLES.ANKLE.X(:,demoofsubjectrow),'linewidth', 4 ,'LineStyle', (stlye{1, 1}), 'color', (farbe{1, 1}))
hold on
plot(DATA.KandK.ProbMeanCurves.Running_Shoe_B.ANGLES.ANKLE.X(:,demoofsubjectrow),'linewidth', 4 ,'LineStyle', (stlye{1, 2}), 'color', (farbe{1, 2}))
plot(DATA.KandK.ProbMeanCurves.Running_Shoe_C.ANGLES.ANKLE.X(:,demoofsubjectrow),'linewidth', 4 ,'LineStyle', (stlye{1, 3}), 'color', (farbe{1, 3}))
...
box off
xlim([1, 161]);
ylabel({'Transversal [°]';'<-Außen-/Innenrot.->'})

figHipAng.Snapshot.Caption =  sprintf('Gelenkswinkel an der Hüfte in allen 3 Raumebenen, die dünnen Linien zeigen dabei den Gruppenmittelwert aller Probanden.');
...
figKneeAng.Snapshot.Caption =  sprintf('Gelenkswinkel am Knie in allen 3 Raumebenen, die dünnen Linien zeigen dabei den Gruppenmittelwert aller Probanden.');
...
set(gca,'xcolor',[1 1 1])
xlim([1, 161]);
ylabel({'Sagittal [°]';'<-Dorsi-/Plantarflex.->'})
subplot(3,1,3)
set(gca,'xcolor',[0 0 0])
...
figAnkleAng.Snapshot.Caption =  sprintf('Gelenkswinkel am Sprunggelenk in allen 3 Raumebenen, die dünnen Linien zeigen dabei den Gruppenmittelwert aller Probanden.');
...
imshow ('C:\Users\maipa\Desktop\IMAGES_REPORT\KAM.png')
figKAMinfo.Snapshot.Caption = sprintf('Kleine Ursache, große Wirkung: Schematische Darstellung, wie Drehmomente am Knie verändert werden können. Durch eine Umorientierung der externen Kräfte (welche auf den Boden aufgebracht werden), kann der Hebelarm bspw. vom Kniegelenk verkleiner werden. Diese führt trotz gleichgebliebener Kraft (vgl. Bild links und Bild rechts) zu einem kleineren Kniedrehmoment. Solche Umorientierungen der Kraft können u.a. durch verschiedene Stützelemente in Laufschuhen erreicht werden.');
...
dirtymatrix=(dirtymatrix(40:120,:));
[M,I] = min(dirtymatrix);
[M,Indexunsed] = min(M);
...
p2 = patch('vertices', [201-20, grenzeny(1,1); 201-20, grenzeny(1,2); 201,grenzeny(1,2) ; 201, grenzeny(1,1)], ...
    ...
    'FaceColor', 'k', ...
    'FaceAlpha', 0.5, 'LineStyle', 'none');
scatter (I(1,Indexunsed)+40,M, 100,'o', 'filled',  'MarkerFaceColor',[255 0 0]/255)
scatter (Imax(1,Indexunsedmax)+40,Mmax, 100,'o', 'filled',  'MarkerFaceColor',[0 128 0]/255)
set(gca,'Fontname','CMU Sans Serif')
set(gca, 'Fontsize', 12)
set(gca, 'linewidth', 2)
box off
...
figKAM.Snapshot.Caption = sprintf('Ihre Knieadduktionsdrehmomente in den drei unterschiedlichen Laufschuhen während des Bodenkontakts mit dem rechten Bein. Der rote Punkt symbolisiert dabei die Schuhbedingung bei der sich Ihr Kniedrehmoment vergrößert hat (negativer). Der grüne Punkt zeigt für welche Schuhbedingung sich Ihr Knieabduktionsdrehmoment verkleinert hat.');
...
elseif Indexunsedmax==2
    schuhebesser = convertCharsToStrings('Schuh B')
end
...
paraKINETIKKAMinfopersonal = Paragraph([(strcat('Die Auswertung zeigt, dass sich Ihr Knieabduktionsmoment ', convertCharsToStrings(schuhe), {' '} ,'gestützten Schuh.', {' '}, schuhebesser, ' wäre also in Bezug auf diesen Verletzungsparameter besser für Sie geeignet. Jedoch möchten wir an dieser Stellen nochmals auf die Komplexität von Überbelastungsverletzungen und die Multifaktorialität von Verletzungsmechanismen verweisen.'))]);
...
figHipMom.Snapshot.Caption = sprintf('Drehmomente an der Hüfte in allen 3 Raumebenen, die dünnen Linien zeigen dabei den Gruppenmittelwert aller Probanden.');
...
figKneeMom.Snapshot.Caption = sprintf('Drehmomente am Knie in allen 3 Raumebenen, die dünnen Linien zeigen dabei den Gruppenmittelwert aller Probanden.');
...
figAnkleMom.Snapshot.Caption = sprintf('Drehmomente am Sprunggelenk in allen 3 Raumebenen, die dünnen Linien zeigen dabei den Gruppenmittelwert aller Probanden.');
