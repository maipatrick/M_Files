...
while i<= 2%length (con_folder)
    ...
    while ab<= length (subject_folders)
        ...
        while zz<= length (datatemp)
            ...
            xminus = -1000000; % relative to lab CS
            xplus = 10000000;% relative to lab CS
            t=1; %loop threough the defindes marker names 
     ...
     if rr== 0
          ...
          txt{f,1}=( ((strcat(datatemp(zz).name,'_', marker{1, t},'_', 'gap'))));
                ...
                t=t+1;
                clearvars value row col value ff lf rr ii jj A
            end
