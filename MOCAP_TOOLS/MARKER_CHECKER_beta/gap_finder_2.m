...
while i<= 1%length (con_folder)
    ...
    while ab<= length (subject_folders)
        ...
        while zz<= length (datatemp)
            ...
            xminus = -5000; % relative to lab CS
            xplus = 3000;% relative to lab CS
%             xminus = -100; % relative to lab CS
%             xplus = 2000;% relative to lab CS
            t=1;
            while t<= length (marker)
                ...
                if volume == 1;
                    ...
                    lf = btkGetLastFrame(acq);
                    %tempiii =  (isnan(  markers.(marker{1, t})(ff:lf,1)));
                    nframes = lf-ff;
                    tempmarkerlength = length ((markers.(marker{1, t})))-1;
                    templuecke = sum(sum (isnan(  markers.(marker{1, t}))));
