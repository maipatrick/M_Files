function [est,HDI,K,XP]=rst_data_plot(Data,varargin)
...
for u=1:grouping
    ...
    if strcmpi(datascatter,'on')
        ...
        for p=1:size(Y,2)
            scatter(X(outliers(:,p)==0,p),Y(outliers(:,p)==0,p),S(find(outliers(:,p)==0)),'k'); hold on
            scatter(X(outliers(:,p)==1,p),Y(outliers(:,p)==1,p),(S(find(outliers(:,p)==1))-(point_size/2)),'k','+');
        end
    ...
    if strcmp(kernel,'on')
        ...
        % plot contours
        y1 = plot(high+gp_index(u),bc); set(y1,'Color',[1 1 1]); hold on
        y2 = plot(low+gp_index(u),bc); set(y2,'Color',[1 1 1]);
        if isnumeric(y1)
        ...
        hold on; fillhandle=fill(xpoints,filled,[1 1 1]);
        set(fillhandle,'LineWidth',0.2,'EdgeColor',[1 1 1],'FaceColor',[0.7 0.7 0.7],'FaceAlpha',0.6,'EdgeAlpha',0);%set edge color
...
% %         plot(xpoints(position,:),filled(position,:),'Color',color_scheme(u,:));
%         plot(xpoints(position,:),filled(position,:),'Color',[1 1 1]);
%         qu = rst_hd(tmp,0.75);
...
plot(X(3:5),repmat(est(u),[1,length(X(3:5))]),'LineWidth',6,'Color',[0.5 0.5 0.5]);
    rectangle('Position',[X(3),HDI(1,u),X(5)-X(3),HDI(2,u)-HDI(1,u)],'Curvature',[0.4 0.4],'LineWidth',1.5,'EdgeColor',[0.5 0.5 0.5])
end
