function [spmi] = PerformSPM_all_only_BETWEEN(SUBJ, A, MATRIXCON, boni, conditions,plane,p, plotall)
...
% figure(111111)
% subplot(3,1,p)
h1= plot(mean(Y1)','linewidth', 4 ,'LineStyle', (stlye{1, 1}), 'color', (farbe{1, 1}));
hold on
h2= plot(mean(Y2)','linewidth', 4 ,'LineStyle', (stlye{1, 2}), 'color', (farbe{1, 2}));
h3= plot(mean(Y3)','linewidth', 4 ,'LineStyle', (stlye{1, 3}), 'color', (farbe{1, 3}));
% h4= plot(mean(Y4)','linewidth', 4 ,'LineStyle', (stlye{1, 4}), 'color', (farbe{1, 4}));
% h5= plot(mean(Y5)','linewidth', 4 ,'LineStyle', (stlye{1, 5}), 'color', (farbe{1, 5}));
% h6= plot(mean(Y6)','linewidth', 4 ,'LineStyle', (stlye{1, 6}), 'color', (farbe{1, 6}));
%h7= plot(mean(Y7)','linewidth', 4 ,'LineStyle', (stlye{1, 7}), 'color', (farbe{1, 7}));
set(gca, 'LineWidth', 2);
...
TF1 = contains(tempylabel,"X");
TF2 = contains(tempylabel,"Y");
TF3 = contains(tempylabel,"Z");
if TF1==1
    tempylabel = (strcat('ant/post GRF', unit));
ylabel (tempylabel)
elseif TF2==1
        tempylabel =(strcat('med/lat GRF', unit));
ylabel (tempylabel)
elseif TF3==1
        tempylabel =(strcat('vertical GRF', unit));
ylabel (tempylabel)
end
...
[cluster] = cluster_extrahieren(Y,spmi);
  ...
  try
    ...
    %%%%%%%%%%%%%%%%%% h4 h5 h6
leg = legend ([h1 h2 h3],cleanconnanes, 'interpreter', 'none','FontSize',30);
set (leg, 'Location', 'Best')
legend boxoff
hold off
% hold off
filename =strrep(strcat(temptitle,joint,plane,'.svg'), '_', ' ');
% filename = (strcat( temptitle,joint,'_' ,plane, '_', '.svg'));
